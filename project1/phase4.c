...
void transfer (int from_id , int to_id , double amount) {
  ...
  int upper = (from_id > to_id) ? from_id : to_id; // predetermines an order.
  int lower = (from_id > to_id) ? to_id : from_id;
...
void* teller_thread(void* arg) {
    ...
    for (int i = 0; i < TRANSACTIONS_PER_TELLER; i++) {
        ...
        // Repeatedly reassigns until 2 different accounts are chosen
        while (randIndex == randIndex2)
        {
            randIndex2 = (rand_r(&seed)) % NUM_ACCOUNTS;
        }
